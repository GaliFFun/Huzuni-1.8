...
public class Speedmine extends BasicMod {
	...
	public void onPacket(PacketEvent event) {
		if (event.type == PacketEvent.Type.SENT && mc.playerController != null && !mc.playerController.isInCreativeMode()) {
			if (event.getPacket() instanceof C07PacketPlayerDigging) {
				...
				} else if (packet.getStatus() == C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK || packet.getStatus() == C07PacketPlayerDigging.Action.ABORT_DESTROY_BLOCK) {
					digging = false;
					this.position = null;
					this.facing = null;
				}
	...
	public void onUpdate(UpdateEvent event) {
		if (event.type == UpdateEvent.Type.PRE) {
			...
			if (digging) {
				IBlockState blockState = this.mc.theWorld.getBlockState(position);
				float multiplier = noSlow.isEnabled() && mc.thePlayer.fallDistance <= 1F
						&& mc.thePlayer.fallDistance > 0 ? 5F : 1F;
				curBlockDamage += blockState.getBlock().getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.theWorld, this.position) * (speed.getValue()) * multiplier;
				if (curBlockDamage >= breakPercent.getValue() / 100F) {
					mc.playerController.onPlayerDestroyBlock(position, facing);
					mc.getNetHandler().addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.position, this.facing));
					curBlockDamage = 0F;
					digging = false;
				}
			}
