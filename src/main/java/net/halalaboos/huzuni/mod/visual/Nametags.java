...
public class Nametags extends BasicMod implements Renderer {
	...
	private void renderNameplate(EntityPlayer entity, double x, double y, double z, double delta) {
		if (!Minecraft.isGuiEnabled() || entity == mc.getRenderViewEntity() || (!invisibles.isEnabled() && entity.isInvisible()))
			return;
		int color = getColor(entity, mc.thePlayer.getDistanceToEntity(entity), huzuni.friendManager.isFriend(entity.getName()), entity.isSneaking());
		...
		String text = (huzuni.friendManager.isFriend(entity.getName()) ? huzuni.friendManager.getAlias(entity.getName()) : entity.getDisplayName().getFormattedText()) + getHealth(entity) + (entity.isInvisibleToPlayer(mc.thePlayer) ? TextColor.BLUE + " [Invisible]" : "") + getPing(entity);
	...
	private void renderItems(EntityPlayer player, float rY) {
		...
		draw3dItem(player.getHeldItem(), (center - halfTotalSize) + itemSize * count + 2, (int) rY - 16, 0);
		if (enchants.isEnabled())
			renderEnchantments(player.getHeldItem(), (center - halfTotalSize) + itemSize * count + 2, (int) rY - 16, 0.5F);
		count++;
		for (int i = 4; i > 0; i--) {
	...
	private void renderHealth(EntityPlayer entity) {
		int originX = -40;
		int originY = -12;
		float maxHealth = (float) entity.getEntityAttribute(SharedMonsterAttributes.maxHealth).getAttributeValue();
