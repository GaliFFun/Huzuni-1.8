...
public class HuzuniPlugins extends HuzuniScreen {
	...
	protected void actionPerformed(GuiButton button) {
		...
		switch (button.id) {
		case 0:
			openFile(huzuni.pluginManager.getPluginFolder());
			break;
		case 1:
	...
	private void openFile(File fileIn) {
		String s = fileIn.getAbsolutePath();

		if (Util.getOSType() == Util.EnumOS.OSX) {
			try {
				Huzuni.LOGGER.info(s);
				Runtime.getRuntime().exec(new String[] {"/usr/bin/open", s});
				return;
			} catch (IOException ioexception1) {
				Huzuni.LOGGER.error((String)"Couldn\'t open file", (Throwable)ioexception1);
			}
		}
			...
			try {
				...
				Huzuni.LOGGER.error((String)"Couldn\'t open file", (Throwable)ioexception);
			}
		}
		...
		try {
			...
			Huzuni.LOGGER.error("Couldn\'t open link", throwable);
			flag = true;
		}

		if (flag) {
			Huzuni.LOGGER.info("Opening via system class!");
			Sys.openURL("file://" + s);
		}
	}
