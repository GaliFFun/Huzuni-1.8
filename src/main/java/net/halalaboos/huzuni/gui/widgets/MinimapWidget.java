...
public class MinimapWidget extends Widget {
    ...
    private final Texture minimap = new Texture("minimap.png"), entity = new Texture("entity.png"), location = new Texture("location.png");
    ...
    private final Mode<String> entityMode = new Mode<>("Entity mode", "Render players in a certain way yo", "Arrows", "Heads");
	...
	public void renderMenu(int x, int y, int width, int height) {
		...
		for (int i = 0; i < mc.theWorld.playerEntities.size(); i++) {
			...
			double angle = MathUtils.wrapDegrees(mc.thePlayer.rotationYaw - (Math.atan2(mc.getRenderManager().viewerPosZ - MathUtils.interpolate(player.prevPosZ, player.posZ, 1F), mc.getRenderManager().viewerPosX - MathUtils.interpolate(player.prevPosX, player.posX, 1F)) * 180.0D / Math.PI));
		...
		if (waypoints.isEnabled()) {
			...
			for (int i = 0; i < localWaypoints.size(); i++) {
				...
				float distance = MathUtils.sqrt((float) (mc.thePlayer.posX - waypoint.getPosition().getX()) * (float) (mc.thePlayer.posX - waypoint.getPosition().getX()) + (float) (mc.thePlayer.posZ - waypoint.getPosition().getZ()) * (float) (mc.thePlayer.posZ - waypoint.getPosition().getZ()));
				double angle = MathUtils.wrapDegrees(mc.thePlayer.rotationYaw - (Math.atan2(mc.getRenderManager().viewerPosZ - waypoint.getPosition().getZ(), mc.getRenderManager().viewerPosX - waypoint.getPosition().getX()) * 180.0D / Math.PI));
	...
	private void renderPlayer(EntityPlayer player, float entitySize) {
		...
		if (entityMode.getSelected() == 0) {
			GlStateManager.rotate(player.rotationYaw - mc.thePlayer.rotationYaw, 0F, 0F, 1F);
			entity.render(-entitySize, -entitySize, entityDiameter, entityDiameter);
			GlStateManager.rotate(-(player.rotationYaw - mc.thePlayer.rotationYaw), 0F, 0F, 1F);
