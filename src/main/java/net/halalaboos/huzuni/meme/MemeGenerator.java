...
public class MemeGenerator extends Thread {
	...
	public void run() {
		try {
			...
			JsonObject object = gson.fromJson(HttpUtil.postMap(new URL(String.format(memeType, memeTypeIndex, memeTypeAmount)), new HashMap<String, Object>(), false).trim(), JsonObject.class);
			...
			for (String meme : memes) {
				JsonObject memeObject = gson.fromJson(HttpUtil.postMap(new URL(String.format(actualMeme, memeIndex, memeAmount, meme)), new HashMap<String, Object>(), false).trim(), JsonObject.class);
