...
public final class CapeManager extends JsonFileHandler {
	...
	private final Map<String, ResourceLocation> capes = new HashMap<>();
	
	private final Map<String, CheckCapeThread> threads = new HashMap<>();
	...
	private class CheckCapeThread extends Thread {
		...
		public void run() {
			try {
				...
				map.put("uuid", uuid);
				int response = Integer.parseInt(HttpUtil.postMap(new URL(url), map, false).trim());
				synchronized (lock) {
					if (response == 1) {
