...
public class BlockRegistryImpl implements BlockRegistry {
	...
	public Collection<Block> getRegisteredBlocks() {
		if (registered == null) {
			...
			for (ResourceLocation resourceLocation : net.minecraft.block.Block.blockRegistry.getKeys()) {
				net.minecraft.block.Block block = net.minecraft.block.Block.blockRegistry.getObject(resourceLocation);
				out.add((Block)block);
			}
			registered = out;
		}
